...
export default async function ThesisViewerPage({
  ...
  return (
    <div className="mx-auto w-full max-w-[1280px] px-6 py-8">
      <Link
        href="/app/history"
        className="mb-4 inline-flex items-center gap-1.5 text-[12px] text-text-2 transition-colors hover:text-slate-900"
      >
      ...
      {/* header */}
      <div className="overflow-hidden rounded-2xl border border-line bg-white shadow-sm p-6">
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            <div className="flex items-center gap-3">
              <span className="grid h-12 w-12 place-items-center rounded-xl bg-[linear-gradient(135deg,#3B82F6,#A78BFA)] font-mono text-[14px] font-bold text-slate-900">
                {thesis.ticker.slice(0, 2)}
              </span>
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="font-mono text-2xl font-extrabold tracking-tight text-slate-900">
                    {thesis.ticker}
                  </h1>
          ...
          <div className="flex flex-col items-end gap-3">
            ...
            <div className="flex gap-2">
              <button
                type="button"
                className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-line bg-white px-3 text-[12px] text-text-2 transition-colors hover:border-line-2 hover:text-slate-900"
              >
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          {thesis.agents.map((a) => {
            ...
            return (
              <div
                key={a.id}
                className="flex flex-col gap-2 rounded-xl border border-line bg-slate-50 p-4"
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-auto font-mono text-[12px] font-bold text-slate-900">
                    {a.confidence}%
                  </span>
                </div>
      ...
      {/* citations */}
      <div className="mt-6 rounded-2xl border border-line bg-card p-6">
        <h2 className="mb-3 text-[15px] font-semibold text-slate-900">Kaynaklar</h2>
        <ul className="grid grid-cols-1 gap-2 md:grid-cols-2">
          {thesis.sources.map((s) => (
            <li
              key={s.id}
              className="flex items-center gap-2.5 rounded-lg border border-line bg-slate-50 px-3 py-2"
            >
              <SourceChip source={s} />
