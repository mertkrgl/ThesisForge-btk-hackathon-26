  ...
  return (
    <div className="mx-auto w-full max-w-[1280px] px-6 py-8">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-5">
        ...
        <button
          type="button"
          className="inline-flex h-10 items-center gap-2 rounded-lg border border-line bg-white px-4 text-[13px] font-medium text-text-2 transition-colors hover:border-line-2 hover:text-slate-900"
        >
      ...
      <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {MOCK_WATCHLIST.map((w) => {
          ...
          return (
            <li
              key={w.ticker}
              className="rounded-2xl border border-line bg-white shadow-sm p-5"
            >
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <Link
                  href="/app/history"
                  className="text-[11.5px] text-text-2 hover:text-slate-900"
                >
                  Geçmiş tezler →
                </Link>
              </div>
