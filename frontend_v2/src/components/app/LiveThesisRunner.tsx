...
export function LiveThesisRunner({
  ...
  return (
    <div className="mx-auto w-full max-w-[1440px] px-6 py-8">
      ...
      {/* control row */}
      <div className="mb-6 grid grid-cols-1 gap-3 rounded-2xl border border-line bg-white shadow-sm p-4 md:grid-cols-[180px_1fr_auto]">
        ...
        <input
          ...
          disabled={running}
          className="h-11 rounded-lg border border-line bg-slate-50 px-3 text-[13px] text-slate-900 placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-[3px] focus:ring-primary/20 disabled:opacity-60"
        />
        <div className="flex gap-2">
          ...
          {done && (
            <button
              type="button"
              onClick={start}
              className="inline-flex h-11 items-center gap-2 rounded-lg border border-line bg-white px-3 text-[13px] font-medium text-text-2 transition-colors hover:border-line-2 hover:text-slate-900"
          ...
          {done && (
            <div className="mt-5 overflow-hidden rounded-2xl border border-violet/30 bg-[linear-gradient(180deg,rgba(167,139,250,0.10),rgba(167,139,250,0.03))] p-5 tf-rise">
              ...
              <p className="mt-2 text-[13.5px] leading-relaxed text-text-2">
                ...
                <span className="text-slate-900">Net pozisyon: tutmaya değer.</span>
              </p>
              <div className="mt-4 flex flex-wrap items-center gap-3">
                ...
                <Link
                  href="/app/history"
                  className="text-[12px] text-text-2 hover:text-slate-900"
                >
                  Geçmiş tezlere ekle →
                </Link>
              </div>
  ...
  return (
    <div className="relative">
      <input
        ...
        disabled={disabled}
        placeholder="TUPRS"
        className="h-11 w-full rounded-lg border border-line bg-slate-50 px-3 font-mono text-[14px] font-semibold uppercase tracking-wider text-slate-900 placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-[3px] focus:ring-primary/20 disabled:opacity-60"
      />
      <div className="absolute left-0 right-0 top-[calc(100%+4px)] z-10 hidden flex-wrap gap-1 group-focus-within:flex">
        {POPULAR.filter((p) => p !== value).map((p) => (
          <button
            ...
            type="button"
            onClick={() => onChange(p)}
            className="rounded border border-line bg-slate-100 px-1.5 py-0.5 font-mono text-[10.5px] text-text-2 hover:text-slate-900"
          >
            {p}
          </button>
        ))}
