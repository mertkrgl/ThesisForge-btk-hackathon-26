  ...
  return (
    <div className="relative">
      <input
        ...
        disabled={disabled}
        placeholder="TUPRS"
        className="h-11 w-full rounded-lg border border-line bg-[#0E1830] px-3 font-mono text-[14px] font-semibold uppercase tracking-wider text-white placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-[3px] focus:ring-primary/20 disabled:opacity-60"
      />
      <div className="absolute left-0 right-0 top-[calc(100%+4px)] z-10 hidden flex-wrap gap-1 group-focus-within:flex">
        {POPULAR.filter((p) => p !== value).map((p) => (
          <button
            ...
            type="button"
            onClick={() => onChange(p)}
            className="rounded border border-line bg-[#0F1A30] px-1.5 py-0.5 font-mono text-[10.5px] text-text-2 hover:text-white"
          >
            {p}
          </button>
        ))}
